...
import Dialer from 'dashboard/components/widgets/conversation/Dialer.vue';
...
} from 'dashboard/constants/contactColumns';
...
const visibleColumnKeys = useStorage(
  'chatwoot_contacts_table_columns',
  DEFAULT_VISIBLE_COLUMNS
);
...
const isAllSelected = computed(() => {
  return (
    ...
    props.selectedContactIds.length === props.contacts.length
  );
});
  ...
  <div class="flex flex-col gap-4">
    <div class="overflow-x-auto border border-n-weak rounded-lg">
      <table class="w-full text-left text-sm whitespace-nowrap">
        <thead
          class="bg-n-alpha-1 text-n-slate-11 font-medium border-b border-n-weak"
        >
          <tr>
        ...
        <tbody class="divide-y divide-n-weak bg-n-background">
          ...
          >
            ...
            <td
              v-if="visibleColumnKeys.includes('email')"
              class="px-4 py-3 text-n-slate-11"
            >
              {{ contact.email }}
            </td>

            <td
              v-if="visibleColumnKeys.includes('phone_number')"
              class="px-4 py-3 text-n-slate-11"
            >
              {{ contact.phoneNumber }}
            </td>
            ...
            <td
              v-if="visibleColumnKeys.includes('company_name')"
              class="px-4 py-3 text-n-slate-11"
            >
            ...
            <td
              v-if="visibleColumnKeys.includes('city')"
              class="px-4 py-3 text-n-slate-11"
            >
            ...
            <td
              v-if="visibleColumnKeys.includes('country')"
              class="px-4 py-3 text-n-slate-11"
            >
              <div
                v-if="contact.additionalAttributes?.country"
              ...
              >
                ...
                />
                <span>{{
                  getCountryName(contact.additionalAttributes?.country)
                }}</span>
              </div>
            </td>

            <td
              v-if="visibleColumnKeys.includes('tags')"
              class="px-4 py-3 text-n-slate-11"
            >
              <div
                v-if="contact.labels"
                class="flex items-center gap-1 flex-wrap max-w-xs"
              >
                <div
            ...
            <td
              v-if="visibleColumnKeys.includes('last_activity_at')"
              class="px-4 py-3 text-n-slate-11"
            >
